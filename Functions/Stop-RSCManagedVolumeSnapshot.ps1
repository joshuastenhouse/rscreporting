################################################
# Function - Stop-RSCManagedVolumeSnapshot - Stopping an on demand snapshot of an RSC Managed Volume
################################################
...
.DESCRIPTION
Only use this function for Stopping/Closing standard Managed Volumes, use Start-RSCOnDemandSnapshot for SLA Managed Volumes and any other object type. It can also be piped a ManagedVolume object from Get-RSCManagedVolumes (see examples).
...
.EXAMPLE
Stop-RSCManagedVolumeSnapshot - ObjectID "0a8f6bc2-dce8-53a8-8e04-6de9293b5a26" -SLADomainID "00000-000000-0000000-00002"
This makes the managed volume (specified by ObjectID) read only by taking a snapshot on the SLADomainID specified.

.EXAMPLE
Get-RSCManagedVolumes | Where {$_.ManagedVolume -eq "YOURMVNAME" | Stop-RSCManagedVolumeSnapshot
