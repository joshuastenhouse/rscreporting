...
# Cluster
# https://xxxxxxxx.my.rubrik.com/clusters/be8cb1b6-c0d6-43d9-af09-950e26cf6e4a/overview
IF ($ObjectType -eq "Cluster"){$RSCObjectURL = $RSCURL + "/clusters/" + $ObjectID + "/overview"}

# SLA domain
# https://xxxxxxxx.my.rubrik.com/sla/details/41450dc4-1021-448e-9861-efe6a7699624
...
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/vsphere/tags/vcenter/f6ba8d25-ccea-579f-b1ac-f3aabda76683
IF ($ObjectType -eq "vCenterTagCategories"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/vsphere/tags/vcenter/" + $ObjectID}

# vCenters
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/vsphere/folders/vcenter/4d06815d-07f9-5465-80a5-18c8149cd2d2
IF ($ObjectType -eq "vCenter"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/vsphere/folders/vcenter/" + $ObjectID}

# vCenter hosts
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/vsphere/hosts/host/46a14e78-b580-5091-b808-0fbb6c0ddb63
IF ($ObjectType -eq "vCenterHost"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/vsphere/hosts/host/" + $ObjectID}

# vCenter Clusters
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/vsphere/hosts/computecluster/e90741cc-4360-54b8-9ad3-84db4727c62e
IF ($ObjectType -eq "vCenterCluster"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/vsphere/hosts/computecluster/" + $ObjectID}

# Nutanix AHV
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/nutanix/VMS/2da128a0-a65c-50d0-9b3f-5dac778d1014/overview
IF ($ObjectType -eq "NutanixVirtualMachine"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/nutanix/VMS/" + $ObjectID + "/overview"}

# HyperV VM
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/hyperV/vms/c2b7064b-0ae1-5752-b43b-f886f7a2bc79/overview
...
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/m365SharepointSiteCollection/7cda68be-4525-4e5c-a39b-ce2a70f1a3cf/overview
...
# 0365 User
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/o365exchange/420eb055-e898-4896-92ce-5cd562c34783/overview
...
# AWS S3Bucket
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/awsNativeS3Bucket/c6cf0547-5949-4133-82c4-cf370aba5d89/overview
...
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/aws/EC2/account/ac0b7687-291e-459f-84aa-901f0777ab1f
IF ($ObjectType -eq "awsEC2account"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/aws/EC2/account/" + $ObjectID}

# AWS RDS account view
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/aws/RDS/account/f8d22d77-8d23-4be3-bdc7-3a31774bdd2d
...
# Azure SQL database
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/azure_sql/db/databases/04e88f39-5982-4c6e-9cca-f238a3c32446/overview
IF ($ObjectType -eq "AZURE_SQL_DATABASE_DB"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/azure_sql/db/databases/" + $ObjectID + "/overview"}

# Azure VM sub
#https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/azure/subscriptions/46341153-5307-4ee4-8b8b-323880589ad9/virtual_machines
...
# Azure SQL sub
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/azure_sql/db/subscriptions/29302ac7-34c3-4691-9da4-175b12dc53cd/databases
...
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/azure/blob/subscriptions/29302ac7-34c3-4691-9da4-175b12dc53cd
...
# GCP Project
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/gcp/project/31b683d4-1c5a-43e4-80d0-1805bc31af8c/gce_instances
...
# ManagedVolume
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/managedVolumes/LEGACY/422e12dd-d948-537b-93f0-4a951f4fdac7/overview
IF ($ObjectType -eq "ManagedVolume"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/managedVolumes/LEGACY/" + $ObjectID + "/overview"}

# SlaManagedVolume
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/managedVolumes/SLA_BASED/315174be-497e-535a-bcbc-1b28aa61814f/overview
IF ($ObjectType -eq "SlaManagedVolume"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/managedVolumes/SLA_BASED/" + $ObjectID + "/overview"}

# SAP Hapa
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/sap_hana/DATABASES/377cbb80-4d0b-598d-bb65-a589009c88cf/overview
IF ($ObjectType -eq "SapHanaDatabase"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/sap_hana/DATABASES/" + $ObjectID + "/overview"}

# SAP Hana System
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/sap_hana/SYSTEMS/f72a707c-f655-5761-936b-740cd62bff54
...
# MssqlHost
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/mssql/hosts_instances/host/922d7801-9551-5cf3-92d8-02a9af468b4c
...
# Oracle Host
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/oracle/hosts_rac/host_rac/30481806-7d37-5af6-9d98-c93e687af7fb
IF ($ObjectType -eq "OracleHost"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/hosts_rac/host_rac/" + $ObjectID}

# Roles
# https://xxxxxxxx.my.rubrik.com/roles/00000000-0000-0000-0000-000000000000
IF ($ObjectType -eq "Role"){$RSCObjectURL = $RSCURL + "/roles/" + $ObjectID}

# SDD Policy
# https://xxxxxxxx.my.rubrik.com/sonar/management/policies/9a21a88c-269f-4037-bfe7-99d906848101
IF ($ObjectType -eq "SDDPolicy"){$RSCObjectURL = $RSCURL + "/sonar/management/policies/" + $ObjectID}

# SDD object
# https://xxxxxxxx.my.rubrik.com/sonar/objects/detail/14c3d9c3-7f3c-57fb-b10c-5f6b491b1974/bfa53fcc-e743-5b8b-8ef2-b15b81b773ba/browse
IF ($ObjectType -eq "SDDObject"){$RSCObjectURL = $RSCURL + "/sonar/management/policies/" + $ObjectID}

# Db2Database
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/db2/DATABASES/d3e73715-fe70-5161-b825-ae60b0c19809/overview
IF ($ObjectType -eq "Db2Database"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/db2/DATABASES/" + $ObjectID + "/overview"}

# Db2Instance
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/db2/INSTANCES/a82babe6-65c2-523b-97b9-3a5a1bb271dd
IF ($ObjectType -eq "Db2Instance"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/db2/INSTANCES/" + $ObjectID + "/overview"}

# K8 Namespace
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/kubernetes/K8Namespace/42a70899-88f8-506d-9aeb-0c4a03e79fc6/overview
IF ($ObjectType -eq "K8SNamespace"){$RSCObjectURL = $RSCURL + "/inventory_hierarchy/kubernetes/K8Namespace/" + $ObjectID + "/overview"}

# AD Domain
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/on_prem_ad/domains/797ea553-a601-5772-b650-7325a8ae3ed2/overview
IF ($ObjectType -eq "ADDomain"){$RSCObjectURL = $RSCURL + "/on_prem_ad/domains/" + $ObjectID + "/overview"}

# AD Domain Controllers
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/on_prem_ad/domain_controllers/5218aae2-cee6-5bcc-b2e8-1348f10f61d9/overview
IF ($ObjectType -eq "ADDomainController"){$RSCObjectURL = $RSCURL + "/on_prem_ad/domain_controllers/" + $ObjectID + "/overview"}

# AD Domain controllers
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/on_prem_ad/domain_controllers/bfcb4867-959e-5329-99a4-e1160937b0a8/overview
IF ($ObjectType -eq "ACTIVE_DIRECTORY_DOMAIN_CONTROLLER"){$RSCObjectURL = $RSCURL + "/on_prem_ad/domain_controllers/" + $ObjectID + "/overview"}

# EntraID domain
# https://xxxxxxxx.my.rubrik.com/inventory_hierarchy/azure_ad/f07e6ce6-2536-40d0-8509-ed6d8c78034f/overview
