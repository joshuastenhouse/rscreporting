################################################
# Function - Get-RSCEventsLogBackupOnDemand - Getting all RSC On-Demand Log Backup events
################################################
Function Get-RSCEventsLogBackupOnDemand {

<#
.SYNOPSIS
Returns all RSC on-demand backup events within the time frame specified, default is 24 hours with no parameters.
...
This example returns all on-demand backup events within a 24 hour period as no paramters were set.

.EXAMPLE
Get-RSCEventsBackupOnDemand -DaysToCapture 30
This example returns all on-demand backup events within a 30 day period.

.EXAMPLE
Get-RSCEventsBackupOnDemand -DaysToCapture 30 -LastActivityStatus "FAILED" -ObjectType "VMwareVirtualMachine"
This example returns all failed on-demand backup events within 30 days for VMwareVirtualMachines.
...
############################
# Only Adding To Array if On-Demand Backup (no other way to pre-filter as of 03/01/23)
############################
