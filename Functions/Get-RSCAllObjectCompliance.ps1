################################################
# Creating the Get-RSCAllObjectCompliance function
################################################
Function Get-RSCAllObjectCompliance {
...
.EXAMPLE
$AllObjectCompliance = Get-RSCAllObjectCompliance
...
Test-RSCConnection
# Getting all protected objects that should be reported on for compliance (I.E not replicas) 
# $ProtectedObjects = Get-RSCObjects | Where-Object {$_.ProtectionStatus -eq "Protected"}
$ProtectedObjects = Get-RSCObjects -ObjectType $ObjectType -Logging | Where-Object {$_.ReportOnCompliance -eq $TRUE}
