################################################
# Function - Write-RSCEventsRecovery - Inserting all RSC Recovery events into SQL
################################################
Function Write-RSCEventsRecovery {

<#
.SYNOPSIS
Collects the event type specified and writes them to an existing MS SQL databse of your choosing, if not specified the default table name RSCEventsRecovery will created (so you don't need to know the required structure).
...
.PARAMETER SQLTable
Not required, it will create a table called RSCEventsRecovery for you, but you can customize the name (not the structure). Has to not already exist on 1st run unless you already used the correct structure. 
...
.EXAMPLE
Write-RSCEventsRecovery -SQLInstance "localhost" -SQLDB "YourDBName"
This example collects all events from the default last 24 hours and writes them into a table named RSCEventsRecovery that it will create on first run with the required structure in the database RSCReprting.

.EXAMPLE
Write-RSCEventsRecovery -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30
This example collects all events from the default last 30 days and writes them into a table named RSCEventsRecovery that it will create on first run with the required structure in the database RSCReprting.

.EXAMPLE
Write-RSCEventsRecovery -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName"
...
.EXAMPLE
Write-RSCEventsRecovery -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName" -DontUseTempDB
...
.EXAMPLE
Write-RSCEventsRecovery -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName" -LastActivityStatus "FAILED"
...
IF($SQLTable -eq $null){$SQLTable = "RSCEventsRecovery"}
# Creating query
...
$EventSeconds = [Math]::Round($EventSeconds,2)
