...
# Deciding if scheduled backup 
IF($EventMessage -match "Scheduled backup"){$IsScheduledBackup = $TRUE}
# Enabling insert
$InsertRow = $TRUE
# If exclude log backups disabling insert
IF(($ExcludeLogBackups) -and ($IsLogBackup -eq $TRUE)){$InsertRow = $FALSE}
# If exclude scheduled disabling insert
IF(($ExcludeScheduledBackups) -and ($IsScheduledBackup -eq $TRUE)){$InsertRow = $FALSE}
