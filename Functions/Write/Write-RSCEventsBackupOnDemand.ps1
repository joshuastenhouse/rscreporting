################################################
# Function - Write-RSCEventsBackupOnDemand - Inserting all RSC Backup events into SQL
################################################
Function Write-RSCEventsBackupOnDemand {

<#
.SYNOPSIS
Collects the event type specified and writes them to an existing MS SQL databse of your choosing, if not specified the default table name RSCEventsBackupOnDemand will created (so you don't need to know the required structure).
...
.PARAMETER SQLTable
Not required, it will create a table called RSCEventsBackupOnDemand for you, but you can customize the name (not the structure). Has to not already exist on 1st run unless you already used the correct structure. 
...
.EXAMPLE
Write-RSCEventsBackupOnDemand -SQLInstance "localhost" -SQLDB "YourDBName"
This example collects all events from the default last 24 hours and writes them into a table named RSCEventsBackupOnDemand that it will create on first run with the required structure in the database RSCReprting.

.EXAMPLE
Write-RSCEventsBackupOnDemand -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30
This example collects all events from the default last 30 days and writes them into a table named RSCEventsBackupOnDemand that it will create on first run with the required structure in the database RSCReprting.

.EXAMPLE
Write-RSCEventsBackupOnDemand -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName"
...
.EXAMPLE
Write-RSCEventsBackupOnDemand -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName" -DontUseTempDB
...
.EXAMPLE
Write-RSCEventsBackupOnDemand -SQLInstance "localhost" -SQLDB "YourDBName" -DaysToCapture 30 -SQLTable "YourTableName" -LastActivityStatus "FAILED"
...
IF($SQLTable -eq $null){$SQLTable = "RSCEventsBackupOnDemand"}
# Creating query
...
----------------------------------
Filtering for On-Demand events & processing..."
################################################
# Processing Events
################################################
...
#
# End of bypass for not being an on-demand backup below
}
# End of bypass for not being an on-demand backup above
#
